<?php
class Xxtest_model extends CI_Model
{
	...
	public function get_locations (){
		...
		public function get_datam($stat, $instr, $para, $nums, $which)
		{
		...
		public function get_datax($which)
		{
			...
			$sql = "SELECT * FROM (SELECT * FROM data_numeric WHERE id_inventory = (SELECT id_inventory FROM inventory WHERE id_instrumentlocation= ? AND id_model=?) AND parameter_id = ? ORDER BY TIMESTAMP DESC LIMIT 1000) T1 ORDER BY TIMESTAMP";
			
			//log_message('info', 'Nums value is  ' . $nums);
			//$this->db->limit($nums);
			log_message('info', 'Called xxtest_model getdatax()');
			$query = $this->db->query($sql, array(1,1,1));
			log_message('info', 'Last query is  ' . $this->db->last_query());
