...
class Scripts_model extends CI_Model
{
	...
	public function get_locations (){
		...
		public function get_data($stat, $instr, $para)
		{
			
			$sql = "SELECT * FROM data_numeric WHERE id_inventory = (SELECT id_inventory FROM inventory WHERE id_instrumentlocation= ? AND id_model=?) AND parameter_id = ? LIMIT 20";
			log_message('info', 'inside scripts_model');
			$query = $this->db->query($sql, array($stat, $instr, $para));
			log_message('info', 'inside scripts_model query called');
			$numdata = '';
			foreach ($query->result_array() as $row)
			{
				log_message('info', 'inside scripts_model inside foreach');
				$temp = $row['timestamp'] . "\t" . $row['value_avg'] . "\n";
				log_message('info', 'inside scripts_model inside foreach $temp = ' . $temp);
				$numdata .= $temp;
				log_message('info', 'get_data returned row ' . $temp);
			}
			return $numdata;
		}
